...
def main():
    ...
    if opt.render:  #test
        ...
        while total_steps < opt.Max_train_steps:
            s, done, steps, ep_r = env.reset(), False, 0, 0     #env.reset(): return initial state. 
            ...
            while not done:         #done = False
                ...
                s_prime, r, done, info = env.step(a)    #go to next state. 
                ...
                replay_buffer.add(s, a, r, s_prime, dw)  # store transition tuple (s,a,r,s') in buffer
                s = s_prime # current state = s'
                ep_r += r  # episode reward update

                '''train if its time'''
                # train 50 times every 50 steps rather than 1 training per step. Better!
                if total_steps >= update_after and total_steps % opt.update_every == 0: #opt.update_every = 50 (default)
